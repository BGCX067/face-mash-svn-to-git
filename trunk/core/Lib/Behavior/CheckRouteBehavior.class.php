...
class CheckRouteBehavior extends Behavior {
    ...
    public function run(&$return){
        ...
        if(!empty($regx)&&C('URL_MODEL')>0){
        	$this->rewrite($_SERVER['PATH_INFO']);
        }
    ...
    private function rewrite(){
    	$arr=D('rewrite/Rewrite')->getValues();
    	foreach($arr as $key=>$value){
    		if(strpos($_SERVER['PATH_INFO'],$key)!==false){
    		 	$_SERVER['PATH_INFO'] = str_replace($key, $value, $_SERVER['PATH_INFO']);
    			//break;
    		}
    	}
